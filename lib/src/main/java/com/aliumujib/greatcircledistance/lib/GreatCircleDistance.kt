...
class GreatCircleDistance(private val parser: ICustomerParser,
    ...
    private fun makeStorageFile(storageURI: String): File {
        ...
        val fileName ="results_${Calendar.getInstance().time.time}"
        return File(file, fileName)
    }

    fun fetchEligibleCustomers(reader: BufferedReader, destinationLatitude: Double, destinationLongitude: Double,  minDistance: Double) : List<Customer>{
        resultStorageDir?.let { storageDir->
            ...
            store.storeResults(customerList, makeStorageFile(storageDir))
    ...
    companion object{
        private val mainInstance = GreatCircleDistance(di().parser, di().store, di().distanceCalc)

        @JvmStatic
        fun getInstance()  = mainInstance

        @JvmStatic
        fun newInstance(parser: ICustomerParser, store: IStore, distanceCalc: IDistanceCalc)  = GreatCircleDistance(parser, store, distanceCalc)
    }
